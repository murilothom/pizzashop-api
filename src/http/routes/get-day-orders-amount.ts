...
export const getDayOrdersAmount = new Elysia()
  ...
  .get('/metrics/day-orders-amount', async ({ getCurrentUser }) => {
    ...
    const ordersPerDay = await db
      ...
      .where(
        and(
          ...
          gte(orders.createdAt, startOfYesteday.toDate()),
        ),
      )
    ...
    const diffFromYesterdayInPercentage =
    ...
    return {
      ...
      diffFromYesterdayInPercentage: diffFromYesterdayInPercentage
        ? Number((diffFromYesterdayInPercentage - 100).toFixed(2))
        : 0,
    }
  })
